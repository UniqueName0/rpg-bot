...
async def arena(ctx,arg):
    ...
    hp = int(hp3)
    dmg1 = rpgdata.execute("select damage from rpgdb where userID=?", (user.id,)).fetchone()
    dmg2 = str(dmg1)
    dmg3 = ''.join(c for c in dmg2 if c.isdigit())
    dmg = int(dmg3)
    defense1 = rpgdata.execute("select armor from rpgdb where userID=?", (user.id,))
    defense2 = str(defense1)
    defense3 = ''.join(c for c in defense2 if c.isdigit())
    defense = int(hp3)
    dodge1 = rpgdata.execute("select evasion from rpgdb where userID=?",  (user.id,))
    dodge2 = str(dodge1)
    dodge3 = ''.join(c for c in dodge2 if c.isdigit())
    dodge = int(dodge3)
    enemyhp = 25*int(arg)
    enemydmg = 5*int(arg)
